...
Как это работает?
1️⃣ Пишем тест (он не пройдёт, потому что кода ещё нет).
2️⃣ Пишем код, который делает тест успешным.
3️⃣ Оптимизируем код, чтобы он был чище, но тест продолжал проходить.
4️⃣ Повторяем процесс на следующей фиче.

Зачем TDD?
✅ Код сразу тестируемый.
✅ Меньше багов.
✅ Можно быстро менять код без страха, что что-то сломается.

[⬆️ Back to Top](#Оглавление)

# DDD
DDD – Разработка, ориентированная на предметную область
DDD – это про грамотное проектирование кода, чтобы он отражал реальный бизнес.

Основные идеи DDD
Бизнес-логика – в центре кода (а не разбросана по контроллерам и моделям).
Используем понятные бизнесу термины.
Чистая архитектура (разделение домена, инфраструктуры и API).

Что даёт DDD?
✅ Код понятен даже бизнес-аналитикам.
✅ Бизнес-логика не размазана по всему проекту.
✅ Легче менять и расширять функциональность.

## TDD-DDD

| **Методология** | **Что делает?** | **Когда применять?** |
|----------------|----------------|---------------------|
| **TDD** | Сначала тесты, потом код | Если нужно меньше багов и простой рефакторинг |
| **DDD** | Код строится вокруг бизнес-логики | Если проект сложный и важно правильно спроектировать |

📌 **Лучший вариант — использовать оба подхода вместе.**  
**TDD** помогает писать качественный код, а **DDD** делает его понятным и логичным.
