...
PHP-FPM (FastCGI Process Manager) — это альтернативный способ обработки PHP-запросов через FastCGI, который значительно улучшает производительность и управление процессами по сравнению с традиционным CGI или модулем Apache mod_ph
...
3️⃣ В зависимости от конфигурации, PHP-FPM может обрабатывать запросы параллельно, обрабатывая несколько запросов одновременно в разных процессах. 4️⃣ PHP-FPM использует FastCGI для связи с веб-сервером, где веб-сервер (например, Nginx) передает запросы PHP-FPM.
...
PHP-FPM — это мощный инструмент для управления PHP-процессами, который значительно повышает производительность и масштабируемость веб-серверов. С его помощью можно эффективно обрабатывать запросы в высоконагруженных приложениях и настроить оптимальную работу PHP на сервере.
...
Абстрактный — это класс, который не может быть инстанцирован(instantiated - создание экземпляра класса) напрямую. Он может содержать абстрактные методы (без реализации), которые должны быть реализованы в дочерних классах
...
Позднее статическое связывание — это механизм в PHP, который позволяет динамически определять класс, из которого вызван статический метод. Работает с ключевым словом static, в отличие от self, которое всегда ссылается на класс, где метод был объявлен.
...
```
✔ Теперь ChildClass::whoAmI() возвращает ChildClass, потому что static::class учитывает, из какого класса был вызван метод.
...
2. Паттерн "Обсервер" (или Observer Pattern, Event/Listener) — это поведенческий паттерн, который позволяет объекту (известному как "субъект") уведомлять другие объекты (известные как "наблюдатели") об изменениях своего состояния. Это используется для реализации механизма подписки, когда один объект может уведомлять другие об изменениях, не зная о них конкретно.

3. Паттерн "Фабрика" (Factory Pattern) — это порождающий шаблон проектирования, который предоставляет общий интерфейс для создания объектов, вместо их прямого создания через new.
   Зачем нужен?
...
4. Паттерн Singleton полезен, когда нужно, чтобы приложение использовало один и тот же экземпляр класса в разных частях программы. Однако его следует использовать с осторожностью, так как он может привести к трудностям с тестированием, расширением и пониманием кода в больших проектах.
...
5. Паттерн Компоновщик (Composite) — это структурный паттерн проектирования, который позволяет создавать древовидные структуры объектов и работать с ними единообразно, как с единичными объектами.
   ### Когда использовать?
...
6. Паттерн Адаптер (Adapter) - это структурный паттерн проектирования, который позволяет объектам с несовместимыми интерфейсами работать вместе. Он создаёт промежуточный слой между клиентом и адаптируемым объектом.
   ### Когда использовать?
...
## JIT 
До появления JIT-компиляции (Just-In-Time) в PHP 8, код выполнялся через интерпретацию байт-кода, с возможностью использования кэша OPcache.
...
### Чем JIT лучше?
JIT-компилятор компилирует PHP-код в нативный машинный код во время выполнения, что значительно ускоряет сложные вычисления.
...
🚀 Вывод: До JIT PHP зависел от OPcache, но всегда работал как интерпретатор. JIT впервые позволил преобразовывать код в машинные инструкции, приближая PHP к компилируемым языкам.
...
## SOLID
SOLID - это набор принципов объектно-ориентированного программирования, которые помогают создавать более гибкий, понятный и поддерживаемый код. Эти принципы были предложены Робертом Мартином и включают:

Вот как расшифровывается акроним SOLID:

S: Single Responsibility Principle (Принцип единственной ответственности).
User, Email
Класс должен иметь одну и только одну причину для изменения. В Laravel, это может означать, что модель отвечает только за работу с базой данных, а контроллер - за обработку запросов.

O: Open-Closed Principle (Принцип открытости-закрытости).
поведение класса должно быть изменяемым без изменения самого класса.
Классы должны быть открыты для расширения, но закрыты для модификации. В Laravel это может быть достигнуто использованием абстрактных классов и интерфейсов.
...
В контексте Dependency Injection (DI), термины "needs" и "give" могут быть использованы для обозначения принципов или механизмов внедрения зависимостей, хотя они не являются стандартной частью самого DI.
...
### Селективность
Селективность индекса — это степень уникальности значений в индексируемом столбце. Чем выше селективность, тем эффективнее индекс, так как он помогает быстрее находить нужные данные.
...
## генераторы
Генераторы в PHP позволяют создавать ленивые итераторы, которые экономят память и вычислительные ресурсы. Вместо того чтобы загружать все данные в память, генератор отдаёт элементы по одному, используя yield.
...
# Дизайн паттерн
Дизайн-паттерны являются важным инструментом для создания гибкого, масштабируемого и поддерживаемого кода. Они решают общие задачи, предоставляя проверенные решения. Разработчики, использующие паттерны, могут эффективнее решать проблемы и создавать более качественное ПО.
...
Паттерны, которые решают проблемы компоновки классов или объектов, обеспечивая удобное представление для создания структуры.
